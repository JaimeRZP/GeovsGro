...
# MCMC CHAIN
######################
def chain(cosmo1, cosmo2, data, command_line):
    """
    ...
    if (data.get_mcmc_parameters(['varying']) != []):
        ...
        sigma_eig, U, C = sampler.get_covariance_matrix(cosmo1, cosmo2, data, command_line)
        ...
        if command_line.fisher:
            sampler.get_fisher_matrix(cosmo1, cosmo2, data, command_line, C, minimum)
            return
    ...
    # to use a jumping factor of 0 with the option "-f 0".
        ...
        data.update_cosmo1_arguments()  # this fills in the fixed parameters
        data.update_cosmo2_arguments()  # this fills in the fixed parameters

        loglike = sampler.compute_lkl(cosmo1, cosmo2, data)
	...
	if command_line.adaptive and k>command_line.adaptive+1:
                ...
                if not (k-1) % (command_line.update/3):
                    try:
                        ...
                        sigma_eig, U, C = sampler.get_covariance_matrix(
                            cosmo1, cosmo2, data, command_line)
                        if command_line.jumping == 'fast':
                            Cholesky = la.cholesky(C).T
        ...
        if get_new_position(
            ...
            newloglike = sampler.compute_lkl(cosmo1, cosmo2, data)
        else:  # reject step
            rej += 1
            if command_line.superupdate:
        ...
        if ((alpha == 1.) or (rd.uniform(0, 1) < alpha)):  # accept step
            ...
            #ZP Store theory prediction. 
            data.xi_theory1_storage.append([data.cosmo1_arguments, "Omega_m1", data.omega_m1 , "Sigma_8_1", data.sigma_8_1, data.xi_theory1])
            data.xi_theory2_storage.append([data.cosmo2_arguments, "Omega_m2", data.omega_m2 , "Sigma_8_2", data.sigma_8_2, data.xi_theory2])
