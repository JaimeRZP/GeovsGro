...
class BOSS_RSD(Likelihood):
#ZP: Modified so only fsigma8 calculations are ran.
        ...
        with open(os.path.join(self.data_directory, self.data_file), 'r') as filein:
            for i, line in enumerate(filein):
                if line.strip() and line.find('#') == -1:
                    ...
                    #ZP: I only want the fsigma8 calculations.
    ...
    def loglkl(self, cosmo1, cosmo2, data):
        #ZP: Being RSD's a
        # a strong growth phenomenon, calculations 
        #are done using cosmo1 as it is the one responsible to 
        #parametrize growth. 
        ...
        for i in range(self.num_bins):
            DM_at_z = cosmo1.angular_distance(self.z[i]) * (1. + self.z[i])
            H_at_z = cosmo1.Hubble(self.z[i]) * conts.c / 1000.0
            rd = cosmo1.rs_drag() * self.rs_rescale
            ...
            #ZP: make the other entries 0 to preserve dimensionality
            #data_array = np.append(data_array, 0) #perfect matching
            #data_array = np.append(data_array, 0) #Perfect matching
