...
class kids450_bs_2cosmos_mark2(Likelihood):
    ...
    def loglkl(self, cosmo1, cosmo2, data):
        ...
        pk = self.cosmo_calculations0(cosmo1, data, np.size(self.xi_obs), cosmo_index = 1)
        xi_theo = self.cosmo_calculations1(cosmo2, pk, data, np.size(self.xi_obs), cosmo_index = 2)
        # final chi2
        vec = xi_theo - self.xi_obs 
        vec = vec[self.mask_indices]
        #print vec, len(vec)
	#print self.xi_obs, len(self.xi_obs)
    ...
    def cosmo_calculations1(self, cosmo, pk ,data, size_xi_obs, cosmo_index = 1):

        # Omega_m contains all species!
        Omega_m = cosmo.Omega_m()
        small_h = cosmo.h()
        sigma_8_1 = cosmo.sigma8()
        data.omega_m1 = Omega_m
        data.sigma_8_1 = sigma_8_1
        ...
        for il in xrange(self.nlmax):
            ...
            for Bin1 in xrange(self.nzbins):
                for Bin2 in xrange(Bin1,self.nzbins):
                    Cl_GG_integrand[:, self.one_dim_index(Bin1,Bin2)] = g[:, Bin1] * g[:, Bin2] / r**2 * pk1[il, :]
                    ...
                    if intrinsic_alignment:
                        ...
                        Cl_II_integrand[:, self.one_dim_index(Bin1,Bin2)] = pr[:, Bin1] * pr[:, Bin2] * factor_IA**2 / r**2 * pk1[il, :]
                        Cl_GI_integrand[:, self.one_dim_index(Bin1,Bin2)] = (g[:, Bin1] * pr[:, Bin2] + g[:, Bin2] * pr[:, Bin1]) * factor_IA / r**2 * pk1[il, :]
        ...
        #data.xi_theory1 = [xi1_theta, xi2_theta]
