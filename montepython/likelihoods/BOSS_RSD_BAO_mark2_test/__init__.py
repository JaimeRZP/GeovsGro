...
class BOSS_RSD_BAO_mark2_test(Likelihood):
	...
	print(data.experiments)
        if 'kids450_bs_2cosmos_mark2' in data.experiments:
	...
	if len(self.error) == 0:
               #print('No extra measurements')
               inv_cov_ext = np.linalg.inv(self.cov_data)
        else:
