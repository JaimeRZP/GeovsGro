...
def run(cosmo1, cosmo2, data, command_line):
    ...
    if command_line.method == 'MH':
        ...
        mn.run(cosmo1, cosmo2, data, command_line)
    elif command_line.method == 'PC':
        import PolyChord as pc
        pc.run(cosmo1, cosmo2, data, command_line)
...
def compute_lkl(cosmo1, cosmo2, data):
    ...
    if ((data.need_cosmo1_update) or
        ...
        try:
            data.cosmo1_arguments['output']
        except:
            data.cosmo1_arguments.update({'output': ''})
        if 'SZ' in data.cosmo1_arguments['output']:
            try:
                if 'SZ_counts':
                    cosmo1.compute(["szcount"])
                else:
                    cosmo1.compute(["szpowerspectrum"])
    ...
    if ((data.need_cosmo2_update) or
        ...
        # the choice of a new point.
        try:
            data.cosmo2_arguments['output']
        except:
            data.cosmo2_arguments.update({'output': ''})
        if 'SZ' in data.cosmo2_arguments['output']:
            try:
                if 'SZ_counts':
                    cosmo2.compute(["szcount"])
                else:
                    cosmo2.compute(["szpowerspectrum"])
            except CosmoComputationError as failure_message:
                sys.stderr.write(str(failure_message)+'\n')
                sys.stderr.flush()
                return data.boundary_loglike
    ...
    # FK: ugly solution below... ZP: As far as I understand, the solution found consists in adding "1" and
    # "2" labels to the parameters which doesn't seem tragic to my sense of aesthetics.
