...
# TODO: prepare total and rejected chain
    ...
    for i in range(ini, 1+n_modes):
        ...
        # Add ACCEPTED points
	np.savetxt('mode_lines.txt', mode_lines, fmt='%s')
	mode_lines = np.loadtxt("mode_lines.txt", dtype=str)
	mode_data = []
	for item0 in mode_lines:
    		if 'E' in item0[1]:
			group = []
			for item1 in item0:
				group.append(float(item1))
           		mode_data.append(group)
	mode_data = np.array(mode_data)

	#mode_data = np.array(mode_lines[i].split(), dtype='float')
        columns = 2+NS_arguments['n_params']
	#mode_data = mode_data.reshape([mode_data.shape/columns, columns])  
