...
class kv450_bs_2cosmos_mark3(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        ##Commented out to implement Benji S. mod
    ...
    def loglkl(self, cosmo1, cosmo2, data):
        ...
        pk_1, pk_lin_1 = self.cosmo_calculations0(cosmo1, data, np.size(self.xi_obs), cosmo_index = 1)
        xi_theo = self.cosmo_calculations1(cosmo2, data, pk_1, pk_lin_1, np.size(self.xi_obs), cosmo_index = 2)

        # final chi2
        vec = xi_theo - self.xi_obs
	vec = vec[self.mask_indices]
	#print vec, len(vec) 
    ...
    def cosmo_calculations0(self, cosmo, data, size_xi_obs, cosmo_index =1):
        ...
        for index_z in xrange(self.nzmax):
            all_k_in_inv_Mpc = (self.l + 0.5) / r[index_z]
            ...
            if any(idx_larger_k_max_in_inv_Mpc):
                ...
                p_dm = np.polyfit(np.log(all_k_in_inv_Mpc[itp_indices]), [np.log(cosmo.pk(all_k_in_inv_Mpc[i], self.z_p[index_z])) for i in itp_indices], 1)
                p_lin_dm = np.polyfit(np.log(all_k_in_inv_Mpc[itp_indices]), [np.log(cosmo.pk_lin(all_k_in_inv_Mpc[i], self.z_p[index_z])) for i in itp_indices], 1)
            for index_l in range(self.nlmax):
                # standard Limber approximation:
                #k = l[index_l] / r[index_z]
                ...
                if 'A_bary' in data.mcmc_parameters:
                    ...
                    pk[index_l, index_z] = pk_dm * self.baryon_feedback_bias_sqr(k_in_inv_Mpc / cosmo.h(), self.z_p[index_z], A_bary=A_bary)
                    # don't apply the baryon feedback model to the linear Pk!
                    #pk_lin[index_l, index_z] = pk_lin_dm * self.baryon_feedback_bias_sqr(k_in_inv_Mpc / cosmo.h(), z[index_z], A_bary=A_bary)
                else:
    ...
    def cosmo_calculations1(self, cosmo, data, pk_inpt, pk_lin_inpt, size_xi_obs, cosmo_index=1):
        ...
        #print 'g(r) \n', self.g
        #ZP: Load previously calculated result. 
        pk = pk_inpt
        pk_lin = pk_lin_inpt
